...
const conversationController = {
  getAllByUser: asyncHandler(async (req, res, next) => {
    try {
      ...
      const conversations_document = await Conversation.find({ 'members.user_id': user_id }, {}).populate({
        path: "last_message"
      }).populate({ path: "members.user_id" }).sort({ 'updatedAt': 'desc' })
      ...
      conversations_document.forEach(async (conversation) => {
        ...
        if (!conversation.is_room) {
          if (conversation.members.length === 2) {
            console.log("user chat: " + conversation.members.length + " " + conversation._id);
            let receiver_document =
              conversation.members[0].user_id == user_id ?
              conversation.members[1] : conversation.members[0]
            ...
            console.log("private chat: " + conversation.members.length + " " + conversation._id);
            // conversations_document.splice(i, 1);
            // i--;
          }
        } else {
          console.log("group chat: " + conversation.members.length + " " + conversation._id);
          // conversations_document.splice(i, 1);
          // i--;
        }
      })
